...
export default function MovieEdit(props) {
  ...
  // Declare url outside fetches, same endpoint but different fetches
  ...
  const fetchMovies = async () => {
    ...
    try {
      ...
      const { title, genre, length, rating, releaseYear } = data.movie;
  ...
  async function handleSubmit(e) {
    ...
    // Sending our token auth through headers, our bodObj, and the method PATCH for the endpoint
    const requestOptions = {
      headers: new Headers({
        Authorization: props.token,
    ...
    try {
      ...
      const data = await res.json();
      // console.log(data);
      // Use navigate on button to go back to table view
    } catch (error) {
      console.error(error);
    }
  }
