...
/* 
    Syntax:
        ...
        IDs: usually a db will create an id automatically per data item
            - IDs are meant to be unique value: we target it because it is unique to the data
*/
...
/* 
    ...
    - save file (means using fs.writeFile to update and save the whole json file/object)
    ...
    * stretch goal: update just the data without modifying the ID
    ...
    [x] Create logic that uses an array method to find and match the param to a db item: forEach
...
router.delete("/:id", (req, res) => {
  try {
    ...
    fs.readFile("./helpers/db.json", (err, data) => {
      ...
      // Takes the JSON content from the file location, parses it out into a plain array with normal JS objects instead of JSON objects
      const db = JSON.parse(data);
